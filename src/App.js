  ...
  render() {
    ...
    if (web3) {
      ...
      for (let e in this.state.events) {
        ...
        if (
          anEvent.sender.toLowerCase() ==
          this.state.metaContract._address.toLowerCase()
        ) {
          console.log("SENDER IS METAACCOUNT, SEARCH FOR ACCOUNT IN FORWARDS!");
          for (let f in this.state.MetaForwards) {
            if (
              this.state.MetaForwards[f].destination ==
              contracts.Stories._address
            ) {
              console.log(
                "FOUND ONE GOING TO THIS CONTRACT:",
                this.state.MetaForwards[f].data,
                this.state.MetaForwards[f].signer
              );
              if (this.state.MetaForwards[f].data.indexOf("0xebaac771") >= 0) {
                ...
                let writeString = this.state.web3.eth.abi.decodeParameter(
                  "string",
                  parts
                );
